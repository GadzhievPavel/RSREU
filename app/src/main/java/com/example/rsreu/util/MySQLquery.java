...
//            " AND weekDay="+weekDay+" AND(dates="+dates+" OR dates=0)"
    ...
    public ArrayList<Numerator>getNumerators(String group,int weekDay,String dates){
            ArrayList<Numerator> numerators = new ArrayList<>();
    ...
    public ArrayList<ArrayList<Numerator>> getNumeratorAll(String group,String firstWeekDay){
        ArrayList<ArrayList<Numerator>> numeratorAll=new ArrayList<ArrayList<Numerator>>();
        for(int i=1;i<7;i++){
            ArrayList<Numerator> numerators =new ArrayList<>();
            Cursor cursor = database.rawQuery("SELECT * FROM numerator" +
                    " WHERE Name=" + group + " AND weekday=" + i + " AND(dates=" + firstWeekDay + " OR dates=0)", null);//firstweekday is zaglushka, need add private metod count days
            cursor.moveToFirst();
            for (int j = 0; !cursor.isAfterLast(); j++) {
                Numerator numerator = new Numerator();
                numerator.setWeekDay(cursor.getInt(1));
                numerator.setTimeId(cursor.getInt(2));
                numerator.setDuration(cursor.getInt(3));
                numerator.setTitle(cursor.getString(4));
                numerator.setType(cursor.getString(5));
            ...
            numeratorAll.add(numerators);
            cursor.close();
        }
        return numeratorAll;
    }
    public ArrayList<ArrayList<Denominator>> getDenominatorAll(String group,String firstWeekDay) {
        ArrayList<ArrayList<Denominator>> allDenominator=new ArrayList<>();
        for (int j = 1; j < 7; j++) {
            ArrayList<Denominator> denominators=new ArrayList<>();
            Cursor cursor = database.rawQuery("SELECT * FROM denominator" +
                    " WHERE Name=" + group + " AND weekDay=" + j + " AND(dates=" + firstWeekDay + " OR dates=0)", null);
            cursor.moveToFirst();
            for (int i = 0; !cursor.isAfterLast(); i++) {
                Denominator denominator = new Denominator();
                denominator.setWeekDay(cursor.getInt(1));
            ...
            cursor.close();
            allDenominator.add(denominators);
        }
        return allDenominator;
    }
